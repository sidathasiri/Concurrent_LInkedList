...
float insertFraction;   //insert operations fraction
float deleteFraction;   //delete operations fraction
int threadCount;        //number of executing threads

struct Node *head_p = NULL;
struct Node **head_pp = &head_p;

pthread_rwlock_t rwlock;    //instance of read write lock
...
int main(int argc, char *argv[]){
    ...
    for(int i=0; i<iterations; i++){
        ...
        pthread_rwlock_destroy(&rwlock);    //destroy lock
        free(thread_handles);   //free aquired memory
    }
